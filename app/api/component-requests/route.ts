...
export async function GET(req: NextRequest) {
  try {
    ...
    if (user.role === "FACULTY") {
      ...
      // Faculty can see requests they've approved or requests for components in their domains
      if (faculty && faculty.domain_assignments.length > 0) {
        ...
        // Faculty without domain assignments can see requests they've approved
        requests = await prisma.componentRequest.findMany({
          where: { 
            approved_by: faculty.id,
            ...(status && { status: status as any })
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user || user.role !== "STUDENT") {
      return NextResponse.json({ error: "Only students can create component requests" }, { status: 403 })
    }
